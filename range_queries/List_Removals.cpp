 ...
 * Solution for: https://cses.fi/problemset/task/1749/
*/
...
ndata combine(ndata l, ndata r) {
    return make_ndata(l.cnt + r.cnt);
}
void build(int idx, int s, int e) {
    if (s == e) {
        segtree[idx] = make_ndata(1);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m);
...
int query(int idx, int s, int e, int pos) {
    if (pos > segtree[idx].cnt) return -1;
    if (s == e) {
        segtree[idx] = make_ndata(0);
        return a[s];
    }
    int m = (s + e)/2;
    int res1 = query(2*idx, s, m, pos); 
    if (res1 != -1) {
        segtree[idx] = combine(segtree[2*idx], segtree[2*idx + 1]);
        return res1;
    }
    int res2 = query(2*idx + 1, m + 1, e, pos - segtree[2*idx].cnt);
    segtree[idx] = combine(segtree[2*idx], segtree[2*idx + 1]);
    return res2;
}
void pre_compute() {
...
void solve() {
    ...
    build(1, 0, n - 1);
    int pos;
    for (int i = 0; i < n; i++) cin >> pos, cout << query(1, 0, n - 1, pos) << " ";
}
