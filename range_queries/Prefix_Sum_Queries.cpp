 ...
 * Solution for: https://cses.fi/problemset/result/11236572/
*/
...
Data merge(Data &l, Data &r) {
    Data res;
    res.sum = l.sum + r.sum;
    res.max_pre = max(l.max_pre, l.sum + r.max_pre);
    return res;
}
Data make_data(ll sum, ll pre_sum) {
    Data res;
    res.sum = sum;
    res.max_pre = pre_sum;
    return res;
...
void update1(int idx, int s, int e, int pos, int val) {
    if (s == e) {
        segtree[idx].sum += val;
        segtree[idx].max_pre += val;
        return;
    }
    int m = (s + e)/2;
    if (pos <= m) update1(2*idx, s, m, pos, val);
    else update1(2*idx + 1, m + 1, e, pos, val);
    segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
}
Data query(int idx, int s, int e, int l, int r) {
    if (s > r || e < l) return make_default();
    if (l <= s && e <= r) {
        return segtree[idx];
    }
    int m = (s + e)/2;
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (t == 1) {
            cin >> pos >> val;
            pos--;
            update1(1, 0, n - 1, pos, val - a[pos]);
            a[pos] = val;
        } else {
            cin >> l >> r;
            l--, r--;
            ll res = query(1, 0, n - 1, l, r).max_pre;
            if (res < 0) cout << 0 << nline;
            else cout << res << nline;
        }
    }
}
