...
ndata segtree[4*mxn];
ndata make_ndata(int val) {
    ndata res;
    res.mn = val;
    return res;
}
ndata combine(ndata l, ndata r) {
    if (l.mn <= r.mn) return make_ndata(l.mn);
    return make_ndata(r.mn);
}
void build(int idx, int s, int e) {
    if (s == e) {
        segtree[idx] = make_ndata(x[s]);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m);
...
ndata query(int idx, int s, int e, int l, int r) {
    if (e < l || s > r) return make_ndata(IINF);
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (type == 1) {
            ...
            cin >> l >> r; l--, r--;
            cout << query(1, 0, n - 1, l, r).mn << nline;
        }
    }
}
