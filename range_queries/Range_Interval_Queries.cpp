 ...
 * Solution for: https://cses.fi/problemset/task/3163/
*/
...
#define N 200005
int n, q;
int x[N], a[N], b[N], c[N], d[N], buf[3*N], ans[2*N], top = 0;
struct query {
    int l, _c, _d, idx;
};
vector<query> f[N];
...
void solve() {
    cin >> n >> q;
    for (int i = 0; i < n; i++) cin >> x[i], buf[top++] = x[i];
    for (int i = 0; i < q; i++) {
        cin >> a[i] >> b[i] >> c[i] >> d[i];
        a[i]--, b[i]--;
        buf[top++] = c[i];
        ...
        f[a[i]].push_back(query{a[i], c[i], d[i], 2*i});

        f[b[i] + 1].push_back(query{b[i] + 1, c[i], d[i], 2*i + 1});
    }
    sort(buf, buf + top);
    top = unique(buf, buf + top) - buf;
    for (int i = 0; i < top; i++) toIdx[buf[i]] = i;
    FenwickTree tree(top);
    for (int i = n - 1; i >= 0; i--) {
        tree.add(toIdx[x[i]], 1);
        for (auto &_q : f[i]) {
            int sidx = toIdx[_q._c];
            int eidx = toIdx[_q._d];
            ans[_q.idx] = tree.sum(sidx, eidx);
        }
    }
    for (int i = 0; i < q; i++) {
        int cans = ans[2*i] - ans[2*i + 1];
        cout << cans << nline;
    }
}
