 ...
 * Solution for: https://cses.fi/problemset/task/1143/
*/
...
int query(int idx, int s, int e, int x) {
    if (segtree[idx].mx < x) return -1;
    if (s == e) {
        return s;
    }
    int m = (s + e)/2;
    int res1 = query(2*idx, s, m, x);
    if (res1 != -1) {
        return res1;
    }
    return query(2*idx + 1, m + 1, e, x);
}
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (res == -1) cout << 0 << nline;
        else {
            cout << res + 1 << nline;
            update(1, 0, n - 1, res, h[res] - x);
            h[res] -= x;
        }
    }
}
