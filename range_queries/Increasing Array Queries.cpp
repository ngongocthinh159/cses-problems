 ...
 * Solution for: https://cses.fi/problemset/task/2416/
*/
...
// https://codeforces.com/blog/entry/132357

#define N 200005
#define LOG 18
int n, q, x[N], par[LOG][N];
...
void solve() {
    ...
    vector<int> R(n);
    for (int i = n - 1; i >= 0; i--) {
        while (st.size() && x[st.back()] <= x[i]) st.pop_back();
        if (st.size())
            R[i] = st.back();
        else
            R[i] = n;
        ...
        if (R[i] != n) {
            int len = R[i] - i;
            ll w = 1LL * x[i] * len - (pref[R[i]] - pref[i]);
            g[R[i]].push_back({i, w});
            g[i].push_back({R[i], w});
        }
    }
    for (int j = 0; j < LOG; j++) par[j][n - 1] = n - 1;
    dfs(n - 1, -1, 0);
    for (int i = 0; i < q; i++) {
        int l, r;
        cin >> l >> r;
        --l, --r;
        int u = l;
        for (int bit = LOG - 1; bit >= 0; bit--) if (par[bit][u] <= r)
             u = par[bit][u];
        // cout << l << ' ' << r << ' ' << u << nline;
        int len = r - u + 1;
        ll ans = sum[l] - sum[u] + 1LL * x[u] * len - (pref[r + 1] - pref[u]);
        cout << ans << nline;
    }
}
