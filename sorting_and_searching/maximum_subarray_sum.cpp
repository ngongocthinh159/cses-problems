...
int main() {
    ...
    // Kadane algorithm
    // Recurence relation: for each index i, the maximum subarray ending with index i
    // will be equal to max(the maximum subarray ending with index i-1, current num in index i)
    // Because: 
    // 1. For subarray ending with index i, it can connect with maximum subarray ending with i-1 to create new subarray
    // 2. Or it will not connect, and the new subarray will be only the element in index i itself
    ll curSum = nums[0];
    ll ans = curSum;
    f(i,1,n) {
