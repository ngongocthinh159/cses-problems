 ...
 * Solution for: https://cses.fi/problemset/task/2183/ 
*/
...
int n;
int a[mxN];
bool canMerge(vector<ll> &cur, vector<ll> &prev) {
    return cur[0] <= prev[1] + 1;
}
vector<ll> merge(vector<ll> &cur, vector<ll> &prev) {
    return {min(cur[0], prev[0]), max(cur[1], prev[1])};
}
void solve() {
    cin >> n;
    for (int i = 0; i < n; i++) {
    ...
    vector<ll> prev = {0,1};
    for (int i = 1; i < n; i++) {
        vector<ll> cur = {prev[0]+a[i], prev[1] + a[i]};
        if (canMerge(cur,prev)) {
            prev = merge(cur,prev);
       } else break;
    }
    cout << prev[1] + 1 << nline;
