 ...
 * Solution for: https://cses.fi/problemset/task/2169
*/
...
// reference: https://codeforces.com/blog/entry/98629

#define MAX 200005

int n;
pair<int,pair<int,int>> a[MAX];
pair<int,pair<int,int>> b[MAX];
int covered[MAX], contain[MAX];
void pre_compute() {
...
bool compare2(pair<int,pair<int,int>> &p1, pair<int,pair<int,int>> &p2) {
    if (p1.first > p2.first) return true;
    if (p1.first == p2.first && p1.second.first < p2.second.first) return true;
    return false; 
}
void solve() {
    ...
    pbds tree, tree2;
    for (int i = 0; i < n; i++) {
        auto it = tree.lower_bound({a[i].second.first, -INT_MAX});
        if (it != tree.end()) covered[a[i].second.second] = tree.size() - tree.order_of_key(*it);
        tree.insert({a[i].second.first, i});
    }
    for (int i = 0; i < n; i++) {
        auto it = tree2.upper_bound({b[i].second.first, INT_MAX});
        if (it != tree2.begin()) {
            it--;
            contain[b[i].second.second] = tree2.order_of_key(*it) + 1;
        }
        tree2.insert({b[i].second.first, i});
    }
