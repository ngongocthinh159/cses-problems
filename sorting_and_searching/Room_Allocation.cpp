 ...
 * Solution for: https://cses.fi/problemset/task/1164
*/
...
const int mxN = 2e5 + 5;
ll n;
vector<vector<int>> a(mxN, vector<int>(3));
void solve() {
    cin >> n;
    for (int i = 0; i < n; i++) {
        cin >> a[i][0] >> a[i][1];
    ...
    map<int,int> res;
    for (int i = 0; i < n; i++) {
        while (q.size() && q.top().first < a[i][0]) {
            aroom.push_back(q.top().second);
            q.pop();
        }
        int roomNum;
        if (aroom.size()) {
