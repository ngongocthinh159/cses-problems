 ...
 * Solution for: https://cses.fi/problemset/task/2168/
*/
...
void solve() {
    ...
    sort(a, a + n, compare1);
    sort(b, b + n, compare2);
    int mxR = -1, mnR = INT_MAX;
    for (int i = 0; i < n; i++) {
        covered[a[i].second.second] = mxR >= a[i].second.first;
        mxR = max(mxR, a[i].second.first);
    }
    for (int i = 0; i < n; i++) {
        contain[b[i].second.second] = mnR <= b[i].second.first;
        mnR = min(mnR, b[i].second.first);
    }
