 ...
 * Solution for: https://cses.fi/problemset/task/1679/
*/
...
void dfs(int u, vector<bool> &visited, vector<bool> &instack, bool &cycle, vector<int> &path) {
    visited[u] = true;
    instack[u] = true;
    for (auto v : g[u]) {
        if (!visited[v]) {
            dfs(v, visited, instack, cycle, path);
        } else if (instack[v]) {
            cycle = true;
        }
    }
...
void solve() {
    ...
    for (int i = 1;i <= n; i++) {
        if (!visited[i]) {
            bool cycle = false;
            vector<int> path;
            dfs(i, visited, instack, cycle, path);
            if (cycle) {
