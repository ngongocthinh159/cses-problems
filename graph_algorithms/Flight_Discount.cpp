 ...
 * Solution for: https://cses.fi/problemset/task/1195/
*/
...
void dijkstra(int src) {
    ...
    while (S.size()) {
        ...
        for (auto [v, w_uv] : g[u]) {
            if (discount) {
                if (dist[v][1] > w_u + w_uv) {
                    S.erase({dist[v][1], {v, 1}});
                    dist[v][1] = w_u + w_uv;
                    S.insert({dist[v][1], {v, 1}});
                }
            } else {
                if (dist[v][0] > w_u + w_uv) {
                    S.erase({dist[v][0], {v, 0}});
                    dist[v][0] = w_u + w_uv;
                    S.insert({dist[v][0], {v, 0}});
                }
                if (dist[v][1] > w_u + w_uv/2) {
                    S.erase({dist[v][1], {v, 1}});
                    dist[v][1] = w_u + w_uv/2;
                    S.insert({dist[v][1], {v, 1}});
                }
