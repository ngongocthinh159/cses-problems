 ...
 * Solution for: https://cses.fi/problemset/task/1676/
*/
...
struct DSU {
    ...
    void unite(int a, int b) {
        ...
        if (pa != pb) {
            ...
            size[pa] += size[pb];
            par[pb] = pa;
            largest_compo_size = max(largest_compo_size, size[pa]);
        }
    }
    int largest_compo() {
        return largest_compo_size;
    }
};
int n, m;
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        dsu.make_set(u);
        dsu.make_set(v);
        if (!dsu.same_set(u, v)) compo--;
        dsu.unite(u, v);
        cout << compo <<  " " << dsu.largest_compo() << nline;
    }
}
