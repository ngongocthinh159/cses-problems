 ...
 * Solution for: https://cses.fi/problemset/task/1111/
*/
...
struct Manacher {
    ...
    bool is_palindrome(int l, int r) {
      ...
      return centerIdx + get_longest(centerIdx, l%2==r%2) - 1 >= r;
    }
  };
...
void solve() {
    ...
    int len = 1, ii = 0;
    for (int i = 0; i < n; i++) {
        if (maximize(len, mana.get_longest(i, true))) {
            ii = i - len/2;
        }
        if (i >= 1 && maximize(len, mana.get_longest(i, false))) {
            ii = i - len/2;
        }
    }
    cout << s.substr(ii, len) << nline;
}
