 ...
 * Solution for: https://cses.fi/problemset/task/2106/
*/
...
void solve() {
    ...
    int n = s.size();
    int l = 0, r = n;
    pair<int,ull> repeatHash;
    while (r - l > 1) {
        int m = l + (r - l)/2;
        bool ok = false;
        vector<pair<int,ull>> v;
        int len = m;
        for (int hi = len - 1; hi < n; hi++) {
            v.push_back(hash(hi - len + 1, len, mxPow));
        }
        sort(all(v));
        for (int i = 1; i < v.size(); i++) {
            if (v[i] == v[i - 1]) {ok = true; repeatHash = v[i]; break;}
        }
    ...
    if (l == 0) cout << -1 << nline;
        ...
        for (int hi = l - 1; hi < n; hi++) {
            if (hash(hi - l + 1, l, mxPow) == repeatHash) {
                cout << s.substr(hi - l + 1, l) << nline; 
                break;
            }
        }
    }
