...
void solve() {
    ...
    vector<pair<ll,ll>> dp(1 << n, {LINF, LINF});
    dp[0] = {1, 0};
    for (int X = 0; X < (1 << n); X++)
        for (int i = 0; i < n; i++) {
            if ((X >> i) & 1) {
                int Y = X - (1 << i);
                if (dp[Y].second + w[i] <= s) {
                    if (dp[X].first > dp[Y].first) {
                        dp[X] = {dp[Y].first, dp[Y].second + w[i]};
                    } else if (dp[X].first == dp[Y].first && dp[X].second > dp[Y].second + w[i]) {
                        dp[X].second = dp[Y].second + w[i];
                    }
                } else {
                    if (dp[X].first > dp[Y].first + 1) {
                        dp[X] = {dp[Y].first + 1, w[i]};
                    } else if (dp[X].first == dp[Y].first + 1 && dp[X].second > w[i]) {
                        dp[X] = {dp[Y].first + 1, w[i]};
                    }
                }
            }
