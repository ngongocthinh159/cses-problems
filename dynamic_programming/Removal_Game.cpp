 ...
 * Solution for: https://cses.fi/problemset/task/1097/
*/
...
int n;
ll a[mxn];
ll dp[mxn][mxn][2]; // dp[i][j][0/1]: maximum score the first/second player can get if the game is player within range [i, j] and the first/second player play first
ll prf[mxn];
void pre_compute() {
...
void solve() {
    ...
    for (int len = 2; len <= n; len++) {
        for (int i = 0; i + len - 1 <= n - 1; i++) {
            j = i + len - 1;
            dp[i][j][0] = max(a[i] + prf[j + 1] - prf[i + 1] - dp[i + 1][j][1], a[j] + prf[j] - prf[i] - dp[i][j - 1][1]);
            dp[i][j][1] = max(a[i] + prf[j + 1] - prf[i + 1] - dp[i + 1][j][0], a[j] + prf[j] - prf[i] - dp[i][j - 1][0]);
        }
    }
    cout << dp[0][n - 1][0] << nline;
}
