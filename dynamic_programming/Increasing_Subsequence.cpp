...
ndata make_data(int mx) {
    ndata res;
    res.mx = mx;
    return res;
}
vector<ndata> segtree;
ndata combine(ndata l, ndata r) {
    return make_data(max(l.mx, r.mx));
}
void build(int idx, int s, int e, vector<int> &v) {
    if (s == e) {
        segtree[idx] = make_data(v[s]);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m, v);
    build(2*idx + 1, m + 1, e, v);
    segtree[idx] = combine(segtree[2*idx], segtree[2*idx + 1]);
}
void update(int idx, int s, int e, int pos, int val) {
    if (s == e) {
        segtree[idx] = make_data(val);
        return;
    }
    int m = (s + e)/2;
...
ndata query(int idx, int s, int e, int l, int r) {
    if (e < l || s > r) return make_data(-IINF);
    if (l <= s && e <= r) return segtree[idx];
    int m = (s + e)/2;
    return combine(query(2*idx, s, m, l, r), query(2*idx + 1, m + 1, e, l, r));
}
void pre_compute() {
...
void solve() {
    ...
    vector<int> dp(mx);
    segtree.resize(4*dp.size());
    build(1, 0, dp.size() - 1, dp);
    for (int i = 0; i < n; i++) {
        int j = v[i] - 1;
        int res = query(1, 0, dp.size() - 1, 0, j - 1).mx;
        update(1, 0, dp.size() - 1, j, (res != -IINF ? res : 0) + 1);
    }
    cout << query(1, 0, dp.size() - 1, 0, dp.size() - 1).mx << nline;
}
