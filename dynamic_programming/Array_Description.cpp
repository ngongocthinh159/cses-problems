...
void solve() {
    ...
    vector<vector<int>> dp(n + 1, vector<int>(m + 1, 0)); // dp[i][j] xet i phan tu dau tien: co bao nhieu cach de phan tu thu i = dung j
    if (a[0] == 0) {
        for (int j = 1; j <= m; j++) {
            dp[1][j] = 1;
        }
    ...
    for (int i = 2; i <= n; i++) {
        if (a[i - 1] == 0) {
            for (int j = 1; j <= m; j++) {
                for (auto &move : moves) {
                    int val = j + move;
                    if (1 <= val && val <= m) dp[i][j] = (dp[i][j] + dp[i - 1][val]) % mod;
                }
            }
        } else {
            for (auto &move : moves) {
                int val = a[i - 1] + move;
                if (1 <= val && val <= m) dp[i][a[i - 1]] = (dp[i][a[i - 1]] + dp[i - 1][val]) % mod;
            }
        }
    }
    ll ans = 0;
