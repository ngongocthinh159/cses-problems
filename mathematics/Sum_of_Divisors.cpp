 ...
 * Solution for: https://cses.fi/problemset/task/1082
*/
...
void solve() {
    cin >> n;
    ll r;
    ll ans = 0, tmp1, tmp2, tmp3, _r, _l;
    ll inv_2 = expo(2, MOD - 2, MOD);
    for (ll l = 1; l <= n;) {
        r = (n / (n / l));
        _r = r%MOD; _l = l%MOD;
        tmp2 = (mod_sub(_r, _l, MOD) + 1) % MOD;
        tmp3 = mod_add(_r, _l, MOD);
        tmp1 = mod_mul(tmp2, tmp3, MOD);
        tmp1 = mod_mul(tmp1, inv_2, MOD);
        tmp1 = mod_mul(tmp1, n/r, MOD);
        ans = (ans + tmp1) % MOD;
        l = r + 1;
    }
    cout << ans << nline;
