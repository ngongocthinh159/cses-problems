 ...
 * Solution for: https://cses.fi/problemset/task/1132/
*/
...
int n;
vector<vector<int>> g;
vector<pair<int,int>> mxChild;
vector<int> farthest_dist;
vector<set<pair<int,int>>> info;
vector<int> ans;
void pre_compute() {
...
void dfs1(int u, int par) {
    ...
    for (auto v : g[u]) {
        if (v != par) {
            dfs1(v, u);
            farthest_dist[u] = max(farthest_dist[u], 1 + farthest_dist[v]);
            info[u].insert({farthest_dist[v], v});
        }
    }
}
void changeRoot(int u, int v) {
    // remove contribution of v
    info[u].erase({farthest_dist[v], v});
    if (info[u].size()) {
        auto it = info[u].end();
        it--;
        farthest_dist[u] = (*it).first + 1;
    } else {
        farthest_dist[u] = 0;
    }

    // add contribution of u to v
    info[v].insert({farthest_dist[u], u});
    farthest_dist[v] = max(farthest_dist[v], farthest_dist[u] + 1);
}
