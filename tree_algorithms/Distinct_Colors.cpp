 ...
 * Solution for: https://cses.fi/problemset/task/1139/
*/
...
#define MAXN 200005
int n, distinct[MAXN], color[MAXN], largest[MAXN], sz[MAXN];
vector<int> g[MAXN];
void pre_compute() {

}
void dfs1(int u, int par) { // for each vertex u, pre-compute the largest[u] = v with v is the biggest size child
    sz[u] = 1;
    int mx_sz = -1;
    largest[u] = -1;
    for (auto v : g[u]) if (v != par) {
        dfs1(v, u);
        sz[u] += sz[v];
        if (maximize(mx_sz, sz[v]))
            largest[u] = v;
    }
}
unordered_set<int> dfs2(int u, int par) {
    unordered_set<int> res = largest[u] == -1 ? unordered_set<int>() : dfs2(largest[u], u);
    for (auto v : g[u]) if (v != par && v != largest[u]) {
        auto tmp = dfs2(v, u);
