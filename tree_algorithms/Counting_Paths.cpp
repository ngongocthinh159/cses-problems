 ...
 * Solution for: https://cses.fi/problemset/task/1136/
*/
...
void dfs(int u, int cur_par, int cur_d) {
    depth[u] = cur_d;
    for (auto v : g[u]) {
        if (v != cur_par) {
            par[v][0] = u;
...
void dfs2(int u, int dir_par, vector<int> &M) {
    for (auto v : g[u]) {
        if (v != dir_par) {
            dfs2(v, u, M);
            effect[u] += effect[v];
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (u == lca_uv) {
            M[v]++;
            if (par[u][0]) M[par[u][0]]--;
        } else {
            M[u]++;
            M[v]++;
            M[lca_uv]--;
            if (par[lca_uv][0]) M[par[lca_uv][0]]--;
        }
    }
    dfs2(1, -1, M);
    for (int i = 1; i <= n; i++) {
